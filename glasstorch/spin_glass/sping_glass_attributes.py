...
class SpinGlassAttributes(BaseModel):
    ...
    master_rng : torch.Generator = None
    quenched_rng : torch.Generator = None
    is_quenched : bool = False
    parent : SpinGlass = None
    ...
    def get_next_random_number(self, quenched=False):
        if quenched:
            if self.quenched_rng is None:
                raise ValueError("Quenched RNG is not initialized.")
        else:
            return torch.rand(1, generator=self.master_rng).item()

    def get_next_quenched_rng(self):
        return torch.Generator().manual_seed(self.get_next_random_number(quenched=False))
    
    def get_coupling_coefficient_variance(self, p, N):
        if self.normalization == "thermodynamic":
            return torch.factorial(torch.tensor(p)) / (N ** (p - 1))
    ...
    def get_coupling_coefficient(self):
        ...
        return torch.normal(0.0, torch.sqrt(variance), generator=self.quenched_rng)
